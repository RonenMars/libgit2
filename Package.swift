...
let package = Package(
    ...
    targets: [
        .target(
            ...
            path: ".",
            exclude: [
                // Root-level non-source dirs
                "CMakeLists.txt", "cmake", "ci", "script",
                "docs", "examples", "tests", "fuzzers", "benchmarks",
                ".github", ".devcontainer", ".vscode",
                // Unused dep directories
                "deps/clar", "deps/winhttp", "deps/ntlmclient",
                // Non-C files in deps (SPM 6.x compiles everything)
                ...
                // Non-C files in src
                "src/libgit2/CMakeLists.txt", "src/libgit2/config.cmake.in",
                "src/libgit2/experimental.h.in", "src/libgit2/git2.rc",
                "src/util/CMakeLists.txt", "src/util/git2_features.h.in",
                // Windows-only source dirs/files
                "src/util/win32",
                "src/util/allocators/win32_leakcheck.c",
                "src/libgit2/transports/winhttp.c",
                "src/libgit2/transports/auth_sspi.c",
                // Disabled transport/stream backends (no HTTPS/SSH/NTLM/GSSAPI)
                ...
                // Unused SHA backends
                "src/util/hash/win32.c", "src/util/hash/win32.h",
